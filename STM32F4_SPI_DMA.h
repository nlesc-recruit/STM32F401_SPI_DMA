/* Custom SPI library
 * This library inherits from the STM32 version and adds DMA support for STM32
 * F401. Additional DMA transfer functions are added. After each transfer DMA is
 * disabled, such that the normal transfer functions still work as well
 */
...
class SPIDMAClass : public SPIClass {
 public:
  ...
  byte DMAtransfer(uint8_t pin, uint8_t _data,
                   SPITransferMode _mode = SPI_LAST);
  uint16_t DMAtransfer16(uint8_t pin, uint16_t _data,
                         SPITransferMode _mode = SPI_LAST);
  void DMAtransfer(uint8_t pin, void *_buf, size_t _count,
                   SPITransferMode _mode = SPI_LAST);
  // the byte type is not a typo, but matches the STM32 SPI library.
  void DMAtransfer(byte pin, void *_bufout, void *_bufin, size_t _count,
                   SPITransferMode _mode = SPI_LAST);
  ...
  void DMAtransfer(void *_bufout, void *_bufin, size_t _count,
                   SPITransferMode _mode = SPI_LAST) {
    return DMAtransfer(CS_PIN_CONTROLLED_BY_USER, _bufout, _bufin, _count,
                       _mode);
  }
