...
void SPIDMAClass::begin(uint8_t _pin) {
  ...
  DMATXConfig.PeriphOrM2MSrcDataSize =
      LL_SPI_GetDataWidth(_hwspi) == LL_SPI_DATAWIDTH_8BIT
          ? LL_DMA_PDATAALIGN_BYTE
          : LL_DMA_PDATAALIGN_HALFWORD;
  DMATXConfig.MemoryOrM2MDstDataSize =
      LL_SPI_GetDataWidth(_hwspi) == LL_SPI_DATAWIDTH_8BIT
          ? LL_DMA_PDATAALIGN_BYTE
          : LL_DMA_PDATAALIGN_HALFWORD;
...
byte SPIDMAClass::DMAtransfer(uint8_t pin, uint8_t _data,
                              SPITransferMode _mode) {
  DMAtransfer(pin, &_data, sizeof(uint8_t), _mode);
  // The returned value should be the received data, but we do not yet support
  // receive
  return 0;
}

uint16_t SPIDMAClass::DMAtransfer16(uint8_t pin, uint16_t _data,
                                    SPITransferMode _mode) {
...
void SPIDMAClass::DMAtransfer(uint8_t pin, void *_buf, size_t _count,
                              SPITransferMode _mode) {
  SPI_TypeDef *_hwspi = getHandle()->Instance;
...
void SPIDMAClass::DMAtransfer(byte pin, void *_bufout, void *_bufin,
                              size_t _count, SPITransferMode _mode) {
